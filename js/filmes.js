...
function criarCarrossel(container, titulo, dados, categoriaSlug) {
  ...
  carrosselDiv.innerHTML = filmesLimitados
    .map(
      (filme) => `
...
async function carregarHeroFilmes() {
  try {
    ...
    if (filme && Object.keys(filme).length > 0) {
      ...
      document.getElementById("heroFilmes").style.backgroundImage =
  ...
  for (const categoria of categorias) {
    try {
      const dados = await getDados(
        `/filmes/categoria/${encodeURIComponent(categoria)}`,
      );
...
async function carregarPaginaFilmes() {
  ...
  if (melhoresFilmes && melhoresFilmes.length > 0) {
    ...
    if (carrossel) {
      carrossel.innerHTML = melhoresFilmes
        .slice(0, 15)
        .map(
          (filme) => `
                <div class="card">
