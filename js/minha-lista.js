...
function criarCarrosselFavoritos(containerId, items, tipo) {
  ...
  if (!items || items.length === 0) {
    container.innerHTML =
      '<div class="loading">Nenhum item favorito ainda. Adicione clicando no coração nos detalhes.</div>';
  ...
  const paginaDestino =
    tipo === "serie" ? "detalhesSerie.html" : "detalhesFilme.html";
...
async function carregarFavoritos() {
  ...
  try {
    ...
    if (favoritosValidos.length !== favoritos.length) {
      ...
      console.log(
        `Lista sincronizada: ${favoritos.length} -> ${favoritosValidos.length} itens válidos`,
      );
    }
    ...
    try {
      const seriesFavoritas = todasSeries.filter((serie) =>
        seriesIds.includes(String(serie.id)),
      );
      criarCarrosselFavoritos(
        "seriesFavoritasCarrossel",
        seriesFavoritas,
        "serie",
      );
    } catch (error) {
    ...
    try {
      const filmesFavoritos = todosFilmes.filter((filme) =>
        filmesIds.includes(String(filme.id)),
      );
      criarCarrosselFavoritos(
        "filmesFavoritosCarrossel",
        filmesFavoritos,
        "filme",
      );
    } catch (error) {
