...
function criarListaFilmes(elemento, dados, tipo = "series") {
  ...
  if (!dados || dados.length === 0) {
    carrossel.innerHTML =
      '<div class="loading">Nenhum conteúdo disponível no momento.</div>';
    return;
  }

  const paginaDestino =
    tipo === "series" ? "pages/detalhesSerie.html" : "pages/detalhesFilme.html";

  carrossel.innerHTML = dados
    .map(
      (item) => `
  ...
  try {
    ...
    if (filme && Object.keys(filme).length > 0) {
      ...
      document.getElementById("heroWatchBtn").onclick = () =>
        alert("Reprodução iniciada (demo)");
...
function criarSections() {
  ...
  const sections = [
    // Primeiro: Lançamentos
    {
      ...
      id: "lancamentosFilmes",
      tipo: "filmes",
      endpoint: "/filmes/lancamentos",
      verMaisHref: "pages/todos-conteudo.html?tipo=filmes&secao=lancamentos",
    },
    {
      title: "Lançamentos Séries",
      id: "lancamentosSeries",
      tipo: "series",
      endpoint: "/series/lancamentos",
      verMaisHref: "pages/todos-conteudo.html?tipo=series&secao=lancamentos",
    },
    ...
    // Depois: Melhores
    {
      ...
      id: "top15Filmes",
      tipo: "filmes",
      endpoint: "/filmes/melhores",
      verMaisHref: "pages/todos-conteudo.html?tipo=filmes&secao=melhores",
    },
    {
      title: "Melhores Séries",
      id: "top15Series",
      tipo: "series",
      endpoint: "/series/melhores",
      verMaisHref: "pages/todos-conteudo.html?tipo=series&secao=melhores",
    },
    ...
    if (section.isGenero) {
      ...
      try {
        ...
        if (itensFiltrados.length > 0) {
          ...
          carrossel.innerHTML =
            '<div class="loading">Nenhum conteudo disponivel neste genero.</div>';
        }
      } catch (error) {
        console.error(
          `Erro ao carregar genero ${section.generoSlug} (${section.generoTipo}):`,
          error,
        );
        const carrossel = elemento.querySelector(".carrossel");
        carrossel.innerHTML =
          '<div class="loading">Erro ao carregar genero.</div>';
      }
      return;
    }
...
async function carregarPorCategoria(categoria, nomeCategoria) {
  ...
  try {
    ...
    if (todosItens.length === 0) {
      ...
      carrossel.innerHTML =
        '<div class="loading">Nenhum conteudo encontrado nesta categoria.</div>';
      return;
    }
    ...
    const carrossel = elementos.categoria.querySelector(".carrossel");
    carrossel.innerHTML =
      '<div class="loading">Erro ao carregar conteudo.</div>';
  }
}
...
function initNavegacao() {
  ...
  const homeLink = document.querySelector(
    '.nav-links-left a[href="/fabulioFrontEnd/"]',
  );
  if (homeLink) {
    homeLink.onclick = (e) => {
      e.preventDefault();
