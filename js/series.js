...
function criarCarrossel(container, titulo, dados, categoriaSlug) {
    ...
    carrosselDiv.innerHTML = seriesLimitadas.map(serie => `
...
async function carregarHeroSeries() {
    try {
        ...
        if (serie && Object.keys(serie).length > 0) {
            ...
            document.getElementById('heroSeries').style.backgroundImage = `url('${destaque.background}')`;
...
async function carregarCategorias() {
    ...
    for (const categoria of categorias) {
        try {
            const dados = await getDados(`/series/categoria/${encodeURIComponent(categoria)}`);
...
async function carregarPaginaSeries() {
    ...
    if (lancamentos && lancamentos.length > 0) {
        ...
        if (carrossel) {
            carrossel.innerHTML = lancamentos.slice(0, 15).map(serie => `
                <div class="card">
                    <a href="detalhesSerie.html?id=${serie.id}">
                        <img src="${serie.poster}" alt="${serie.titulo}" onerror="this.src='https://via.placeholder.com/300x450?text=FABULIO'">
                    </a>
    ...
    if (melhoresSeries && melhoresSeries.length > 0) {
        ...
        if (carrossel) {
            carrossel.innerHTML = melhoresSeries.slice(0, 15).map(serie => `
                <div class="card">
                    <a href="detalhesSerie.html?id=${serie.id}">
                        <img src="${serie.poster}" alt="${serie.titulo}" onerror="this.src='https://via.placeholder.com/300x450?text=FABULIO'">
                    </a>
