...
function search(query) {
  ...
  let resultados = todosDados.filter((item) => {
    ...
    const sinopseMatch =
      item.sinopse && item.sinopse.toLowerCase().includes(termo);
    const atoresMatch =
      item.atores && item.atores.toLowerCase().includes(termo);
    const diretorMatch =
      item.diretor && item.diretor.toLowerCase().includes(termo);
...
function destacarTexto(texto, termo) {
  ...
  return texto.replace(
    regex,
    '<span style="color: #F5C16C; font-weight: 700;">$1</span>',
  );
}
