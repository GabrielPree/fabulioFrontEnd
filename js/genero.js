...
function criarCarrossel(containerId, dados, tipo) {
  ...
  if (!dados || !Array.isArray(dados) || dados.length === 0) {
    container.innerHTML =
      '<div class="loading" id="nenhumConteudo">Nenhum conteúdo encontrado para este gênero.</div>';
    return;
  }

  const dadosLimitados = dados.slice(0, 15);
  const paginaDestino =
    tipo === "filme" ? "detalhesFilme.html" : "detalhesSerie.html";

  container.innerHTML = dadosLimitados
    .map(
      (item) => `
...
async function carregarGenero() {
  ...
  try {
    ...
    try {
      const filmesResponse = await getDados(
    ...
    try {
      const seriesResponse = await getDados(
