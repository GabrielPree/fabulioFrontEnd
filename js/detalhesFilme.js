...
function formatarGeneros(generos) {
    ...
    return generos.map(g => g.charAt(0).toUpperCase() + g.slice(1).toLowerCase()).join(', ');
}
...
async function carregarFilme() {
    ...
    try {
        ...
        // Dados para o hero
        document.getElementById('sinopse-filme').textContent = filmeData.sinopse || 'Sinopse em breve.';
        document.getElementById('avaliacao').textContent = filmeData.avaliacao || 'N/A';
        document.getElementById('atores').textContent = filmeData.atores || 'Informações em breve';
        
        // Dados para a box de informações
        document.getElementById('diretor').textContent = filmeData.diretor || 'Informações em breve';
        document.getElementById('ano').textContent = filmeData.anoLancamento || filmeData.ano || 'N/A';
        document.getElementById('duracao').textContent = formatarDuracao(filmeData.duracao);
        document.getElementById('generos').textContent = formatarGeneros(filmeData.generos);
        document.getElementById('avaliacao-info').textContent = `${filmeData.avaliacao || 'N/A'}/10`;
        document.getElementById('atores-info').textContent = filmeData.atores || 'Informações em breve';
        document.getElementById('sinopse-info').textContent = filmeData.sinopse || 'Sinopse em breve.';
...
async function carregarRecomendados(generos) {
    ...
    try {
        ...
        if (recomendados.length === 0) {
            carrossel.innerHTML = '<div class="loading">Nenhum filme recomendado encontrado.</div>';
            return;
        }
        
        carrossel.innerHTML = recomendados.map(filme => `
