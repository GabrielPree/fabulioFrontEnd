  ...
  try {
    ...
    if (generosElement) {
      if (serieData.generos && serieData.generos.length > 0) {
        generosElement.textContent = serieData.generos
    ...
    if (anoElement) {
      anoElement.textContent =
        serieData.anoLancamento || serieData.ano || "N/A";
    }
    ...
    if (infoAvaliacao)
      infoAvaliacao.textContent = `${serieData.avaliacao || "N/A"}/10`;

    const infoGeneros = document.getElementById("info-generos");
    if (infoGeneros) {
      if (serieData.generos && serieData.generos.length > 0) {
        infoGeneros.textContent = serieData.generos
          .map((g) => g.charAt(0).toUpperCase() + g.slice(1).toLowerCase())
          .join(", ");
      } else {
    ...
    if (infoAno)
      infoAno.textContent = serieData.anoLancamento || serieData.ano || "N/A";

    const infoAtores = document.getElementById("info-atores");
    if (infoAtores)
      infoAtores.textContent = serieData.atores || "Informações em breve";

    const infoSinopse = document.getElementById("info-sinopse");
    if (infoSinopse)
      infoSinopse.textContent = serieData.sinopse || "Sinopse em breve.";
...
async function carregarTemporadas() {
  try {
    ...
    if (!temporadas || temporadas.length === 0) {
      document.getElementById("episodios-container").innerHTML =
        "<p>Nenhuma temporada disponível no momento.</p>";
      return;
    }

    const temps = [...new Set(temporadas.map((ep) => ep.temporada))].sort(
      (a, b) => a - b,
    );

    const tempBar = document.getElementById("tempBar");
    tempBar.innerHTML = temps
      .map(
        (temp) => `
    ...
    document.getElementById("episodios-container").innerHTML =
      '<div class="loading">Erro ao carregar episódios.</div>';
  }
}
...
async function carregarEpisodios(temporada) {
  try {
    const episodios = await getDados(
      `/series/${serieId}/temporadas/${temporada}`,
    );
    ...
    if (!episodios || episodios.length === 0) {
      container.innerHTML =
        '<div class="loading">Nenhum episódio encontrado para esta temporada.</div>';
      return;
    }
